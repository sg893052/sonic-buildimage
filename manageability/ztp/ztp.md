...
| v1.0 | 03/22/2020 | Rajendra Dendukuri | Added: Dynamic port breakout support for ZTP, config-db plugin enhancements |
...
26. On switch platforms that support dynamic port breakout, ZTP must try all supported port breakout modes to discover a link up.
27. Certain switch platforms require port fec configuration to be set to a specific mode for the port link to come up. ZTP must try all supported port fec modes to discover a link up.
...
Each section has a unique name, *snmp* in above example. It provides sufficient data to configure a single or a set of modules on the switch. In the *snmp* example, the read write SNMP community string and SNMP location are provided as values of the  *community-ro* and *snmp-location* objects. The ZTP service invokes the logic which takes these values and adds them to "/etc/sonic/snmp.yml" file and restarts SNMP daemon.
...
- **description**: Optional free form textual string used to describe a configuration section defined in the ZTP JSON file.
...
- **start-timestamp** : Specifies the date and time when the ZTP service began processing the configuration section. This object is also available for the top level ztp section and it indicates the date and time when the ZTP service started. This object is used to calculate the processing time. The ZTP service adds this object to the ZTP JSON file 

- **timestamp** : Specifies the date and time when the *status* variable of a section is modified.
...
The following is the list of objects supported by the configdb-json object. Also provided is brief description of their usage, values that can be assigned and the default value assumed when the object is not used.
...
When the configdb-json plugin is executed, the DHCP address assigned during ZTP discovery is released. So it is important that interface IP address assignment is performed as part of the provided *config_db.json* file.

It has been observed that sometimes users are not careful when handling the inventory and configuration files of their switches. There is no central place where they have the mapping information of all the switches, their mac addresses and H/W SKU strings. As a result when config_db.json files are created for a fleet of switches, they tend to have the same DEVICE_METADATA even if the rest of PORT, INTERFACE and other configurations are unique and accurate. When these somewhat accurate configuration files are pushed to switches, the mac address, hwsku and platform string appears to be the same on all switches. This behavior is undesirable. To overcome this problem, the configdb-json plugin overwrites the platform, mac and hwsku fields in the DEVICE_METADATA|localhost entry of the JSON configuration file pushed by the user via ZTP. The corresponding accurate values are read back from current running configuration in the ConfigDB and used. If the DEVICE_METADATA table is missing in the user supplied config_db.json, the DEVICE_METADATA table values in the ConfigDB are inserted into the user supplied config_db.json.
...
The ZTP service is defined as a systemd service running on native SONiC O/S. It does not run inside a container.  ZTP service starts after the *interfaces-config.service*, *rc-local.service* and *database.service*. If ZTP is not administratively enabled, the service exits and does not run again until next boot or if user intervenes. There are no other systemd services that are blocked for ztp.service to start or exit.



The switch tries to obtain management connectivity after it has booted up and all the connected ports are linked up. DHCP discovery is performed on all the connected in-band interfaces and the out-of-band management interface. Also, both DHCPv4 and DHCPv6 address discovery is performed. Whichever interface receives the first DHCP offer is used as primary management interface to obtain user provided provisioning data. 
...
#### 3.7.1 Dynamic port configuration of in-band interfaces

ZTP over in-band interfaces poses a catch-22 situation when the interfaces may have to be configured with a specific configuration for the ports to link up. ZTP can perform discovery and provisioning information only on link up interfaces. However, specific configuration for an interface is known only after the configuration is downloaded after the discovery of provisioning information. To break out from this paradox, the ZTP services dynamically configures each of the in-band interface with all possible predefined configuration options. The supported dynamic port configuration options are listed below:

1. Port Breakout Configuration
2. Forward Error Correction settings (FEC)

The ZTP services reads supported port settings from the platform configuration files and tries them on each of the in-band ports until a port link up is seen. The port FEC settings are modified every 10s (discovery interval). The port breakout mode is modified periodically every 60s (port breakout interval), until the port or any of its expanded ports link up. Note that once a port link up is seen, no additional changes are made to the port configuration. This allows ZTP services to perform DHCP discovery over the active interfaces.

The discovery interval and port breakout interval can be modified by specifying appropriate values in the */host/ztp/ztp_cfg.json*. Use *"discovery-interval" : new_value * and  *"port-breakout-interval" : new_value* in */host/ztp/ztp_cfg.json*. The order in which the individual port configurations are tried is randomized to allow a fair chance for two interconnected switches simultaneously performing dynamic port configuration to arrive at a common configuration value for the link between them to come up.

The dynamic port configuration features can be disabled by setting *"feat-fec-auto-tuning" : false* and/or  *"feat-auto-port-breakout" : false* in the ZTP configuration file */host/ztp/ztp_cfg.json.

The dynamic port configuration as determined by the ZTP services is also stored in the file */host/ztp/ztp_port_cfg.json*. This stored configuration is used to establish connectivity if the switch reboots as part of the initial configuration steps before a full switch configuration is downloaded and applied.
...
When a new SONiC image is installed, the contents of the */etc/sonic* directory are migrated to the newly installed directory. ZTP JSON and ZTP configuration files are also available to the new image as they are stored in */host/ztp* directory. If the image upgrade happened as part of a ZTP session in progress, after booting the new image, ZTP resumes from the point where it left of prior to image switchover. ZTP service waits for configuration migration to complete before taking any action. If after configuration migration, if */etc/sonic/config_db.json* file is not found, the ZTP service creates a ZTP configuration that enables all in-band interfaces and performs DHCP discovery on them to obtain connectivity and resume processing the ZTP JSON file. This establishes connectivity to external hosts for provisioning to be completed.

There can also be a scenario where on a switch a ZTP is in completed (SUCCESS/FAILED) state. A new SONiC image is installed using *sonic_installer* upgrade tool and the user reboots the switch to boot into new image. In this scenario, contents of */host/ztp* and thus ZTP JSON, ZTP configuration file are accessible to the newly installed image. Since ZTP is in completed state, it will not run again. Only if */etc/sonic/config_db.json* file does not exist, the ZTP service creates a ZTP configuration and starts the ZTP procedure afresh. It is up to the user to install configuration migration hooks to migrate changes to new image. More information is available in the [SONiC Configuration Setup Service](https://github.com/rajendra-dendukuri/SONiC/blob/config_setup/doc/ztp/SONiC-config-setup.md) design document.
...
1.  The SONiC switch boots and the ZTP service starts if there is no startup configuration file */etc/sonic/config_db.json*
2.  The DHCP server provides IP connectivity to the management interface along with the DHCP Option 67, which provides a URL to the ZTP JSON file
3. The ZTP service downloads the ZTP JSON file and sequentially processes each of the individual configuration sections according to their lexical sort order.
4. The plugin scripts for each configuration section are executed. User-defined plugins are first downloaded, then executed.
...
All possible scenarios are not described here. Refer to the [ZTP Service](#35-ztp-service) section for additional information.
...
To support HTTPS URLs, the server's SSL certificate, as issued by a trusted certificate authority, must be installed on the switch as part of the default image at build time. */etc/ssl/certs* is the directory to install these certificates. This can be done using organizational extensions or by placing them in the */usr/lib/ztp/certs*  directory inside SONiC ZTP package source code directory.

### 5.2 File Permissions

Only *root* user is allowed to read and modify the files created by the ZTP service. This includes all downloaded files and generated temporary files during ZTP activity.
...
This sub-section explains the list of the configuration options available for ZTP.  The following are the supported ZTP configuration commands. All configurable parameters are found in the *ztp_cfg.json*.  The ZTP service reads the configuration file during initialization.
...
Use this command to manually restart a new ZTP session.  This command deletes the existing */etc/sonic/config_db.json* file and starts the ZTP service. It also erases the previous ZTP session data. The ZTP configuration is loaded on to the switch and ZTP discovery is performed. This command is useful to restart ZTP after it has failed or has been disabled by the user.
...
## 8. Logging
All output generated by ZTP service is logged to local syslog and stdout of the service. The *stdout* and *stderr* of executed plugins are are also redirected to syslog and stdout of ZTP service. In addition to this, logs are are also sent to */var/log/ztp.log* file. User can modify */host/ztp/ztp_cfg.json* to increase or decrease logging priority level. User can also disable logging to file */var/log/ztp.log* or change to which file logs can be written to. Default logging level is assumed to be of level INFO. All messages with severity of INFO or greater are logged.
...
1. Using the below example ZTP JSON file, the following steps are performed on first boot:
...
You can get code coverage report of the ZTP service code by starting the ZTP service with coverage enabled. Modify /etc/default/ztp to set COVERAGE="yes".
...
31. Verify that port FEC settings are continuously modified until a port link up is seen

32. Verify that on a platform that supports dynamic port breakout, ZTP automatically breaks out the ports to detect a link up
...
**Objective:** Verify that port FEC settings are continuously modified until a port link up is seen

**Test Steps:**

- Initiate a ZTP session on a switch with few port links connected and few down
- Ensure that certain ports link up only when a specific fec setting is configured on them

**Expected Results:**

- FEC settings of ports which are down continuously changes from "none", "fc" and "rs"
- FEC settings of ports which are up does not change
- FEC settings of ports which need to be set to a specific value also link up after the ZTP services tries all possible combinations

**Additional Tests:**

- Verify that the order in which fec modes are set is random
- Allow ZTP process to start and configure it reboot the switch as part of the first configuration section even before a valid config_db.json is pushed. The switch should boots up with the previously determined fec settings and can resume ZTP.
- Verify that if a port which was link up goes down, the cycle of trying out all fec modes starts again

### Test Case #32

**Objective:** Verify that on a platform that supports dynamic port breakout, ZTP automatically breaks out the ports to detect a link up

**Test Steps:**

- Initiate a ZTP session on a switch with few port links connected and few ports down. These ports should support dynamic port breakout configuration.
- Ensure that certain ports link up only when a specific port breakout configuration is applied.

**Expected Results:**

- Port breakout settings of ports which are down continuously changes between their supported port modes
- Port breakout settings of ports are up does not change
- Port breakout settings of ports which need to be set to a specific value also link up after the ZTP services tries all possible combinations

**Additional Tests:**

- Verify that the order in which port breakout configuration modes are set is random
- Verify that if a port which was link up goes down, the cycle of trying out all possible port breakout modes starts again
- Allow ZTP process to start and configure it reboot the switch as part of the first configuration section even before a valid config_db.json is pushed. The switch should boots up with the previously determined port breakout configuration and can resume ZTP.
