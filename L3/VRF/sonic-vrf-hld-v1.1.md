...
- [SONiC VRF support design spec draft](#sonic-vrf-support-design-spec-draft)
  ...
  - [Functionality](#functionality)
    ...
    - [Loopback interface support per VRF](#loopback-interface-support-per-vrf)
...
| v1.2    | 07/12/2019 | Preetham Singh | Loopback interface support per VRF |
...
8. Add/Delete Loopback interfaces per VRF. Support to add IPv4/IPv6 host address on Loopback interface.
...
![](images/Multi-VRF_Deployment.png)
...
#### Loopback interface support per VRF
Since Linux kernel supports only one Loopback interface per Net Name Space, VRF implementation using L3 Master Dev(L3MDEV) cannot get per VRF Loopback interface support.
To Meet requirements 8 above, below changes are proposed:
...
So in this implementation release we use VR object as VRF object.
...
![](images/VRF_HIGH_LEVEL_DIAGRAM.png)
...
For Loopback interface support per VRF, only Loopback0 interface IP will be applied on default system loopback interface - "lo". For all non Loopback0 interfaces defined, intfmgrd will be creating a netdevice of type dummy in respective VRF.
For version upgrade compatibility we need to update db migrator script to convert old config-db schema to new config-db schema for loopback interfaces as well.
...
The following is high level event flow diagram from config_db to app_db.
![](images/vrf_event_diagram_1.png)
...
![vrf-event-flow-diagram2](images/vrf_event_diagram_2.png)
...
//bind an interface to a VRF, interface_name can be Ethernet/PortChannel/Vlan/Loopback 
$ config interface vrf bind <interface_name> <vrf_name>

//unbind an interface from a VRF, interface_name can be Ethernet/PortChannel/Vlan/Loopback
...
Below are new CLIs added to support per VRF Loopback interface:

```bash
// create loopback per VRF:
$ config loopback add Loopback<1-999>

// delete loopback:
$ config loopback del Loopback<1-999>
```
Note: We can create/delete loopback interface using above CLI.
...
$ config interface vrf unbind Ethernet0 Vrf-blue 
...
$ config interface vrf bind Loopback10 Vrf-blue
```
...
In intforch
- When app-intf-prefix-table sees IP address add for Loopback interface, vrf name is fetched from local interface cache to obtain VRF-ID and add IP2ME route with this VRF ID.

### Remove IP address on Loopback interface:
```bash
$ config interface ip remove Loopback10 10.1.1.1/32
```
This command does following operations:
In intfmgr:
- When IP address delete is received on these loopback interfaces, ip address will be deleted from corresponding kernel loopback netdevice. Also add {interface_name:ip address} to app-intf-prefix-table.
In intforch
- When app-intf-prefix-table sees IP address delete for Loopback interface, vrf name is fetched from local interface cache to obtain VRF-ID and delete IP2ME route with this VRF ID.

Note: Deleting all ip address on loopback will not delete Loopback interface itself or its binding to VRF. To Delete Loopback interface, user needs to explicitly delete Loopback interface.
