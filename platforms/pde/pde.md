...
The SONiC PDE is generated from a fully compiled SONiC build and is intended to provide ODM and external customers with a means to quickly add, compile and test their platform drivers and static device data required for a fully functional SONiC distribution.  
...
1.  The SONiC PDE repository is generated from a successfully compiled SONiC build.
2.  The SONiC PDE source code tree is generated by a "make" command and generates a PDE tarball which are then given to an ODM for development.
3.  The SONiC PDE is packaged with a user provided list of SONiC supported ODMs and platforms. If no user list is provided, all existing SONiC supported ODM platforms and hardware SKUs are included.
...
![PDE](images/sonicbuildcontainers.png)
