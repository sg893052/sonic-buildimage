...
No Application level code changes needed. However, Docker control scripts have to be enhanced to ensure docker exists when any application is terminated.
...
All docker limit values will be centralized in a JSON file under /etc/sonic/docker-limits.json. See Appendix for details.
Assign cpu-shares value for each Docker.
A wrapper script (sonic-dockerlimits) will return the limits in a string that can directly be used with the ‘docker create’ command.  

     Usage:
      sonic-dockerlimits [ -mem | -cpu | -sys | -update | -updateall ] <container name> 
     For e.g.
      sonic-dockerlimits -sys snmp
      Output:  “--cpus=7 --cpu-shares=900 --memory=10g ”

Edit each docker’s service initialization script which starts the docker

      docker create $(sonic-dockerlimits -sys <dockername> ) <...existing options>

NOTE: The above initialization has already been added to all dockers’ init scripts in MR7 itself. Users are not required to take any action. 


## Docker / Process Restartability
SONiC already has systemd / supervisord based restartability for both Docker and process terminations. This infrastructure already supports restarting dependent Dockers when a critical Docker exits.  As part of rate-limiting changes, each Docker owner has to decide whether the Docker should be restarted or just a process that was killed to be restarted. 

These changes have to be done for all Dockers/processes. 

Scope of changes for Docker restart
The below gerrit change was done for ICCPD.  This can be used a s reference to make the config changes:
     
      http://gerrit-lvn-07.lvn.broadcom.net:8083/c/sonic/sonic-buildimage/+/23400
...
The sysmonitor.sh is already enhanced to alert the user when different memory thresholds are crossed.  Apart from generating appropriate syslogs at each level,  a tech-support dump is collected as well. These changes are under review in the following gerrit id:
      ...
      http://gerrit-lvn-07.lvn.broadcom.net:8083/c/sonic/sonic-buildimage/+/24678
...
| Value.    | Description.          |
|:----------|:----------------------| 
| "auto".   | sonic-dockerlimits script will dynamically compute this based on the device its running |
...
Option 2:  The debugSh infrastructure allows each application to instrument self-termination based on an external signal. The details of implementing this infrastructure is explained in the this document
...
```
lvnvdb2241:/projects/csg_sonic/gh891339/dlimits> ls -l
total 21
-rwxr-xr-x 1 gh891339 users 8848 Oct 15 15:01 cpuhog
-rwxr-xr-x 1 gh891339 users  614 Oct 15 15:01 cpuhog.c
-rwxr-xr-x 1 gh891339 users 8872 Oct 15 15:01 memhog
