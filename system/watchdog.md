...
- Hardware watchdog shall be supported in all the ODM hardware platform devices.
...
### Config commands
- The Watchdog support is enabled by default. There is no KLISH CLI is required. This is a mandatory feature, it should not be disabled at any cause. 
...
- The hardware watchdog designed to use the intel iTCO watchdog driver. This driver is supported on most of the ODM hardware platforms. The watchdog framework consists of following components:
	...
	- Systemd service
	- Watchdog application daemon and utility tools a
...
- These files are preinstalled during the image build itself. When kernel boots up, the iTCO watchdog driver check for previous reboot reason by reading the watchdog status register and update the status according and initialize the watchdog hardware. During the system bootup, it starts the watchdog service as the first service and that starts the watchdog application daemon for watchdog time out refresh. 
                                                         ...
                                                         +--------------------+ 
                                                                  ...
                                                                  |
        +-------------------------------+   +---------------------V--------------------+
        |        Watchdog Daemon        |   |                Watchdog Util             |
        |        (watchdogd)            |   |               (wdctl)                    |
        +------------+------------------+   +-----------------+---------+--------------+
        |  Refresh   |  enable/disable  |   |  enable/disable | counter | reboot cause |
        +------------+------------------+   +-----------------+---------+--------------+
                  |              |                    |              ^            ^
                  |              +<-------------------+              |            |
        User      |              |                                   |            |
...
- The iTCO driver is patched to send the reboot status userspace. This makes the driver to uniform interface to the application  for reading the reboot status. 
...
- The watchdog interface shall be implemented such a way to support both 2.0 platform driver interface as well as  non 2.0 variant APIs. 
- For all the 2.0 platforms, the platform-specific drived class shall be implemented. 
...
## Non-Supported Hardware boards:
 -For the older system that doesn't support watchdog functionality, it should silently leave the application watchdog service in an inactive state. 
...
# Unit Test
  ...
  | SNO |  Unit Testcase 
 :------| :----------------------------------------------------
	1 | Verify the watchdog reset by killing the application process.
	2 | Verify the reboot reason after system bootup with watchdog timeout. 
	3 | Verify the watchdog enable/disable.
