...
#### 3.2.3 Verify that with different global and interface level config of Link Flap Error Disable, always interface level config is preferred.  
...
#### 3.2.13 Verify the Vlan Autostate status when vlan member port is suppressed/released by Link Flap Error Disable.
...
#### 3.2.14 Verify that with different Link Flap Error Disable interface level configs on multiple interfaces, each port gets suppressed/released as per that specific interface configuration.
