...
TCP_FLAGS is changed to a list of values in SONiC 3.1.1 to support *established* keyword in IPv4 and IPv6 ACL. On upgrade from previous versions TCP flags will be converted to a list of 1 element and on downgrade any rules with more 1 value/mask (Used by *established* keyword) will be removed. 
...
this release. This configures the IP address, qos-mode (uniform/pipe) and DSCP value of the VTEP.
DSCP value to be configured only when qos-mode is configured and is mandatory when qos-mode is 
configured as pipe. 
...
Priority-group, Queue Threshold configuration is defined in **THRESHOLD\_TABLE**, and Buffer pool Threshold configuration defined in **THRESHOLD\_BUFFERPOOL\_TABLE** table. The key for a **THRESHOLD\_TABLE** entry is {buffer, buffer\_type, port\_alias, buffer\_index}, and key for a **THRESHOLD\_BUFFERPOOL\_TABLE** entry is {pool_name}. The attributes for both these tables are {threshold: value in percentage}.
...
```
{
	...
	"queue|multicast|CPU|27": {
            "threshold": "24"
        }
    },
    "THRESHOLD_BUFFERPOOL_TABLE": {
        "ingress_lossless_pool": {
            "threshold": "40"
        },
        "egress_lossless_pool": {
...
TAM infrastructure is common for Telemetry features, they are spread into **TAM_SWITCH_TABLE, TAM_COLLECTORS_TABLE, TAM_FEATURES_TABLE, TAM_SAMPLINGRATE_TABLE, TAM_FLOWGROUP_TABLE** tables. The flowgroup configuration for **TAM** feartures comes via **ACL_TABLE|TAM and ACL_RULE|TAM|*** tables.

```
{
    "TAM_SWITCH_TABLE|global": {
        "enterprise-id": "4413",
        "switch-id": "53261"
    }
}
...
{
    "TAM_FEATURES_TABLE|DROPMONITOR": {
        "status": "INACTIVE"
    },
    "TAM_FEATURES_TABLE|IFA": {
        "status": "ACTIVE"
    },
    "TAM_FEATURES_TABLE|TAILSTAMPING": {
        "status": "ACTIVE"
    }
}

{
    "TAM_SAMPLINGRATE_TABLE|s1": {
        "sampling-rate": "5"
    }
}

{
    "TAM_FLOWGROUP_TABLE|f1": {
        "id": "2",
        "table-name": "TAM"
    },
    "TAM_FLOWGROUP_TABLE|f2": {
        "id": "3",
        "table-name": "TAM"
    },
    "TAM_FLOWGROUP_TABLE|f3": {
        "id": "52",
        "table-name": "TAM"
    },
    "TAM_FLOWGROUP_TABLE|f4": {
        "id": "88",
        "table-name": "TAM"
    },
    "TAM_FLOWGROUP_TABLE|f5": {
        "id": "177",
        "table-name": "TAM"
    }
}
...
{
    "ACL_RULE|TAM|f1": {
        "DST_IP": "50.50.50.2/32",
        "IN_PORTS@": "Ethernet46",
        "IP_PROTOCOL": "17",
        "IP_TYPE": "IPV4ANY",
        "PACKET_ACTION": "MONITOR_DROPS",
        "PRIORITY": "100",
        "SRC_IP": "20.20.20.2/32"
    },
    "ACL_RULE|TAM|f2": {
        "DST_IP": "10.10.1.2/32",
        "IP_PROTOCOL": "17",
...
IFA session configuration uses **TAM_IFA_SESSIONS_TABLE** table. Refer **TAM** infrastructure to create 'collector', 'flowgroup' and 'sample-rate'.

```
{
  "TAM_IFA_SESSIONS_TABLE|ifa1": {
    "collector": "c1",
    "flowgroup": "f2",
    "node-type": "EGRESS"
  },
  "TAM_IFA_SESSIONS_TABLE|ifa2": {
    "flowgroup": "f3",
    "node-type": "INGRESS",
    "sample-rate": "s1"
  }
...
The drop monitor feature global and session configuration is defined in **TAM_DROPMONITOR_TABLE|global**. Refer **TAM** infrastructure to create 'collector', 'flowgroup' and 'sample-rate'.


```
{
    "TAM_DROPMONITOR_TABLE|global": {
        "aging-interval": "30"
    }
}

{
    "TAM_DROPMONITOR_SESSIONS_TABLE|mod1": {
        "collector": "c1",
        "flowgroup": "f1",
        "sample-rate": "s1"
    }
...
IFA session configuration uses **TAM_TAILSTAMPING_SESSIONS_TABLE** table. Refer **TAM** infrastructure to create 'flowgroup'.

```
{
    "TAM_TAILSTAMPING_SESSIONS_TABLE|ts1": {
        "flowgroup": "f4",
        "node-type": "IFA"
    },
    "TAM_TAILSTAMPING_SESSIONS_TABLE|ts2": {
        "flowgroup": "f5",
        "node-type": "NORMAL"
    }
}
...
The snapshot interval configuration is defined in **SNAPSHOT_TABLE**, snapshot telemetry interval configuration is defined in **WATERMARK_TABLE**.
...
REST Serever configurations are defined in **REST_SERVER** table.
Only one row can be created with name *default*. This is an optional configuration. 
REST Server will run with default configurations when the *REST_SERVER|default* entry does not exist.
...
### SSH Server
SSH Serever VRF configurations are defined in **SSH_SERVER_VRF** table.
...
Switch resources for the given feature are defined in **SWITCH\_RESOURCE\_TABLE**. The DROP_MONITOR feature supports the scale parameter for flows and supports NONE and MIN flows.
```
{
...
Dynamic Port Breakout (DPB) configuration are defined in three tables **BREAKOUT_CFG, BREAKOUT_PORTS and PORT**.

BREAKOUT_CFG - Break information
key : master port 
brkout_mode : current break out mode 
lanes : Hardware physical lanes
port : standard-name represents front panel port

BREAKOUT_PORTS - Master and sub port relation
key : sub port 
master : master port

PORT - Refer port table 
...
IP ECMP LOADSHARE configurations will be stored in CONFIG DB.

Below is the config-db schema for the IP ECMP LOADSHARE configuration.


```
;New table
;holds the IPV4 ECMP LOADSHARE configuration

key = ECMP_LOADSHARE_TABLE_IPV4|ipv4

; field = value
ipv4_src_ip = IPv4 Source ip address  ; Upon enabling this field, we include ipv4_src_ip in ECMP hash configuration. default=True
ipv4_dst_ip = IPv4 Destination ip address  ; Upon enabling this field, we include ipv4_dst_ip in ECMP hash configuration. default=True
ipv4_l4_src_port = IPv4 L4 Source Port  ; Upon enabling this field, we include ipv4_l4_src_port in ECMP hash configuration. default=True
ipv4_l4_dst_port = IPv4 L4 Destination Port  ; Upon enabling this field, we include ipv4_l4_dst_port in ECMP hash configuration. default=True
ipv4_protocol = IPv4 Protocol  ; Upon enabling this field, we include ipv4_protocol in ECMP hash configuration. default=True

;New table
;holds the IPV6 ECMP LOADSHARE configuration

key = ECMP_LOADSHARE_TABLE_IPV6|ipv6

; field = value
ipv6_src_ip = IPv6 Source ip address  ; Upon enabling this field, we include ipv6_src_ip in ECMP hash configuration. default=True
ipv6_dst_ip = IPv6 Destination ip address  ; Upon enabling this field, we include ipv6_dst_ip in ECMP hash configuration. default=True
ipv6_l4_src_port = IPv6 L4 Source Port  ; Upon enabling this field, we include ipv6_l4_src_port in ECMP hash configuration. default=True
ipv6_l4_dst_port = IPv6 L4 Destination Port  ; Upon enabling this field, we include ipv6_l4_dst_port in ECMP hash configuration. default=True
ipv6_next_hdr = IPv6 Next Header  ; Upon enabling this field, we include ipv6_next_hdr in ECMP hash configuration. default=True

;New table
;holds the IP ECMP LOADSHARE HASH SEED configuration

key = ECMP_LOADSHARE_TABLE_SEED|hash

; field = value
ecmp_hash_seed = ECMP Hash Seed value  ; Allow the user to set specific ECMP hash seed value. default=10
```
